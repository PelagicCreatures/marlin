...
function getUserForRequestMiddleware (marlin) {
	const db = marlin.db
...
function touchToken (db, token, cb) {
	const now = Date.now()
	const accessed = new Date(token.lastaccess).getTime()
	const elapsedSeconds = (now - accessed) / 1000
...
function expireTokens (marlin) {
	...
	cron.schedule('0 */2 * * *', function () {
		...
		marlin.db.getModel('Token').destroy(query)
			.then((result) => {
